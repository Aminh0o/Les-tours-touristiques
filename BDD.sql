...
(
   ...
   ID_COMPTE            varchar(10) not null,
   ID_EMETTEUR          varchar(10) not null,
   COM_ID_COMPTE        varchar(10) not null,
   NOM                  text,
   PRENOM               text,
   LOGIN                varchar(20) not null,
   MOT_DE_PASSE         varchar(10) not null,
...
(
   ID_COMPTE            varchar(10) not null,
   ID_ADMIN_HEBERG      varchar(10) not null,
   ADM_ID_ADMIN_HEBERG  varchar(10) not null,
   ID_ADMIN_TRANSP      varchar(10) not null,
   ADM_ID_ADMIN_TRANSP  varchar(10) not null,
   ID_ADMIN_REST        varchar(10) not null,
   ADM_ID_ADMIN_REST    varchar(10) not null,
   ID_GUIDE             varchar(10) not null,
   GUI_ID_GUIDE         varchar(10) not null,
   NOM                  text not null,
   LOGIN                varchar(20) not null,
   MOT_DE_PASSE         varchar(10) not null,
...
(
   ...
   RATING               numeric(8,0) not null,
   ADRESSE              text not null,
   TELEPHONE            numeric(8,0) not null,
...
(
   NUMEROPACK           int not null,
   ID_RESERVATION       varchar(10) not null,
   RES_ID_RESERVATION   varchar(10) not null,
   TYPE                 varchar(10) not null,
...
(
   ID_RECRUTEMENT       varchar(10) not null,
   ID_UTILISATEUR       varchar(10) not null,
   UTI_ID_UTILISATEUR   varchar(10) not null,
...
(
   ID_UTILISATEUR       varchar(10) not null,
   ID_RECRUTEMENT       varchar(10) not null,
   REC_ID_RECRUTEMENT   varchar(10) not null,
   ID_EMETTEUR          varchar(10) not null,
   NOM                  text,
   DATE_DE_NAISSANCE    date,
   PRENOM               text,
...
alter table ADMIN_HEBERGEMENT add constraint FK_POSSEDE foreign key (COM_ID_COMPTE)
      references COMPTE (ID_COMPTE) on delete restrict on update restrict;

alter table ADMIN_HEBERGEMENT add constraint FK_POSSEDE4 foreign key (ID_COMPTE)
      references COMPTE (ID_COMPTE) on delete restrict on update restrict;
...
alter table ADMIN_RESTAURATION add constraint FK_POSSEDE11 foreign key (COM_ID_COMPTE)
      references COMPTE (ID_COMPTE) on delete restrict on update restrict;

alter table ADMIN_RESTAURATION add constraint FK_POSSEDE9 foreign key (ID_COMPTE)
      references COMPTE (ID_COMPTE) on delete restrict on update restrict;
...
alter table ADMIN_TRANSPORT add constraint FK_POSSEDE3 foreign key (COM_ID_COMPTE)
      references COMPTE (ID_COMPTE) on delete restrict on update restrict;

alter table ADMIN_TRANSPORT add constraint FK_POSSEDE7 foreign key (ID_COMPTE)
      references COMPTE (ID_COMPTE) on delete restrict on update restrict;

alter table ADMIN_TRANSPORT add constraint FK_RECEVER2 foreign key (ID_EMETTEUR)
      references NOTIFICATION (ID_EMETTEUR) on delete restrict on update restrict;

alter table COMPTE add constraint FK_POSSEDE1 foreign key (ADM_ID_ADMIN_TRANSP)
      references ADMIN_TRANSPORT (ID_ADMIN_TRANSP) on delete restrict on update restrict;

alter table COMPTE add constraint FK_POSSEDE10 foreign key (ADM_ID_ADMIN_REST)
      references ADMIN_RESTAURATION (ID_ADMIN_REST) on delete restrict on update restrict;

alter table COMPTE add constraint FK_POSSEDE12 foreign key (ID_GUIDE)
      references GUIDE (ID_GUIDE) on delete restrict on update restrict;

alter table COMPTE add constraint FK_POSSEDE14 foreign key (GUI_ID_GUIDE)
      references GUIDE (ID_GUIDE) on delete restrict on update restrict;

alter table COMPTE add constraint FK_POSSEDE2 foreign key (ADM_ID_ADMIN_HEBERG)
      references ADMIN_HEBERGEMENT (ID_ADMIN_HEBERG) on delete restrict on update restrict;

alter table COMPTE add constraint FK_POSSEDE5 foreign key (ID_ADMIN_HEBERG)
      references ADMIN_HEBERGEMENT (ID_ADMIN_HEBERG) on delete restrict on update restrict;

alter table COMPTE add constraint FK_POSSEDE6 foreign key (ID_ADMIN_TRANSP)
      references ADMIN_TRANSPORT (ID_ADMIN_TRANSP) on delete restrict on update restrict;

alter table COMPTE add constraint FK_POSSEDE8 foreign key (ID_ADMIN_REST)
      references ADMIN_RESTAURATION (ID_ADMIN_REST) on delete restrict on update restrict;
...
alter table GUIDE add constraint FK_POSSEDE13 foreign key (ID_COMPTE)
      references COMPTE (ID_COMPTE) on delete restrict on update restrict;

alter table GUIDE add constraint FK_POSSEDE15 foreign key (COM_ID_COMPTE)
      references COMPTE (ID_COMPTE) on delete restrict on update restrict;
...
alter table PACK add constraint FK_CONCERNER foreign key (RES_ID_RESERVATION)
      references RESERVATION (ID_RESERVATION) on delete restrict on update restrict;

alter table PACK add constraint FK_CONCERNER2 foreign key (ID_RESERVATION)
      references RESERVATION (ID_RESERVATION) on delete restrict on update restrict;

alter table RECRUTEMENT add constraint FK_DEMANDER foreign key (UTI_ID_UTILISATEUR)
      references UTILISATEUR (ID_UTILISATEUR) on delete restrict on update restrict;

alter table RECRUTEMENT add constraint FK_DEMANDER2 foreign key (ID_UTILISATEUR)
      references UTILISATEUR (ID_UTILISATEUR) on delete restrict on update restrict;

alter table RESERVATION add constraint FK_CONCERNER3 foreign key (NUMEROPACK)
      references PACK (NUMEROPACK) on delete restrict on update restrict;

alter table RESERVATION add constraint FK_CONCERNER4 foreign key (PAC_NUMEROPACK)
      references PACK (NUMEROPACK) on delete restrict on update restrict;

alter table UTILISATEUR add constraint FK_DEMANDER3 foreign key (ID_RECRUTEMENT)
      references RECRUTEMENT (ID_RECRUTEMENT) on delete restrict on update restrict;

alter table UTILISATEUR add constraint FK_DEMANDER4 foreign key (REC_ID_RECRUTEMENT)
      references RECRUTEMENT (ID_RECRUTEMENT) on delete restrict on update restrict;
