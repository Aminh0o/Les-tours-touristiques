...
<body>
				...
				<div class="notifications-card">
					...
					<ul class="notif-list">
						<?php 
								...
								while($tuple = $info->fetch(PDO::FETCH_ASSOC))
								{
									...
									echo    "   <li class='notif-item'>
									                ...
									                <div class='notif-contenu'>
										                <p class='notif-message'>".$tuple["MESSAGE_NOTIF"]."</p>
									                </div>
								                </li>
											";
								}
							...
							echo "<script> var notifBadge = document.getElementById('notifBadge');
							notifBadge.innerHTML = '".$nb_notifs."'; </script>";
						?>
					</ul>
				</div>
			</div>
	...
	<?php 
		...
		if(isset($_POST["nomComment"]) && isset($_POST["emailComment"]) && isset($_POST["messageComment"]) && isset($_POST["rating"]))
		{
			...
			$id_avis = $connexion->lastInsertId();

			$notif_avis = "INSERT INTO NOTIF(ID_EMETTEUR,ID_RECEPTEUR,MESSAGE_NOTIF) VALUES ('$id_user',99,'un avis envoyer par utilisateur $id_user')";
			$connexion->exec($notif_avis);

			$laisser = "INSERT INTO LAISSER(ID_UTILISATEUR,ID_AVIS) VALUES ('$id_user','$id_avis')";
			$connexion->exec($laisser);
		}
	?>
...
<script>
	...
	{
		notifBadge.innerHTML = notificationCount;
